...
const result = transformToLis(inputObject);
console.log(result);
...
const count = (target, input) => {
  return input.reduce((count, array) => {
    return count + array.reduce((subCount, number) => {
      return subCount + (number === target ? 1 : 0);
    }, 0);
  }, 0);
};

const exampleInput = [[1, 3, 5, 7, 9], [5, 5, 5], [1, 2, 3]];
const result = count(5, exampleInput);
...
const totalSum = (input) => {
  return input.reduce((sum, array) => {
    return sum + array.reduce((subSum, number) => {
      return subSum + number;
    }, 0);
  }, 0);
};

const exampleInput = [[1, 2, 3, 4, 5], [6, 7, 2, 4, 5, 7], [9, 2, 3, 6]];
const result = totalSum(exampleInput);
...
const exampleInput = [[0, 2, 5, 4], [2, 4, 10], []];
const result = divisibleByFiveTwoToThePower(exampleInput);
console.log(result); // Output: [ [1, 32], [1024], [] ]
...
let starWarsData = [
  // ... (provided Star Wars data) ...
];

const result = findMaleAndFemale(starWarsData);
console.log(result); // Output: "Luke Skywalker and Darth Vader and Leia Organa"
...
const result = findShortest(starWarsData);
console.log(result); // Output: "R2-D2"
